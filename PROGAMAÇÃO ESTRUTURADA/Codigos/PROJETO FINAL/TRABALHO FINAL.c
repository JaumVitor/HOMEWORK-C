...
#define MAX_C 100
	...
	typedef struct {
		char nome[50], cpf[12], end[50], email[50]; 
		int telefone, quant_contas; 
	}cliente;
...
int validarConta (int *cont, int numAgencia, int numConta, conta listaConta[]){ //Valida pelo numAgencia, e numConta
...
int validarContaAgencia (int *cont, int numAgencia, conta listaConta[]){ //Valida apelas pelo numAgencia
	//Retorna 0; se tiver valores de contas iguais.....retorna 1; caso contrario
    int i; 
   	for (i=0; i< *cont; i++){
...
int return_id (int *pos, char auxCliente[], cliente listaCliente[]){ //pecorrendo pelo cpf na lista Cliente
...
int return_id_conta (int *cont, int *numAgencia, int *numConta, conta listaConta[]){ //Pecorrendo pela Agencia e Conta, lista conta
...
int return_id_conta_cpf (int *cont, char cpf[], conta listaConta[]){ //Pecorrendo os cpf das contas
...
void inf_Conta (cliente listaCliente[], conta listaConta[], int *pos, int *cont){ 
	...
	do{
		...
		case 3: //Remover conta, deixando não alterando os valores da lista cliente
			...
			printf ("\n\tVERIFICANDO EXISTENCIA DA SUA CONTA"); carregamento(); systemLinha(); 
			
			if (retorno == 0){
				printf ("\tFAZENDO CANCELAMENTO DA CONTA"); carregamento(); systemLinha(); 
...
void inf_Relatorio (cliente listaCliente[], conta listaConta[], int *pos, int *cont, FILE *salvar, FILE *salvar1, FILE *salvar2, FILE *salvar3){
	...
	do{
		...
		case 6: 
			...
			break; 
		case 0: 
			printf ("\n\tSALVANDO ARQUIVOS");  carregamento(); 
		    system("cls"); break ; 	 
		default: 
			printf ("\nOPS, VALOR DIGITADO NÃO ESTA NO MENU :(\n\n");  
			system("pause"); system("cls");
...
void CadastrarCliente (cliente listaCliente[MAX_C], int *pos, int *cont){ //MENU PRINCIPAL > CADASTRAR
	...
	if (retorno != 0){
		...
		listaCliente[*pos].quant_contas = 0; //Zerando a quantidade de contas 
		++(*pos); 
		systemLinha(); 
	}else{
...
void CadastrarConta (cliente listaCliente[MAX_C], conta listaConta[], int *pos, int *cont){ //MENU PRINCIPAL > CADASTRAR
	...
	if (retorno == 0){ //Cliente precisa ter uma cadastro, para criar sua conta 
